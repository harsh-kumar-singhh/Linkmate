    ...
    const handleSavePost = async (statusArg: "DRAFT" | "SCHEDULED" | "PUBLISHED") => {
        ...
        try {
            ...
            queryClient.setQueryData(["dashboard"], (old: any) => {
                ...
                // This ensures "instant" even if they haven't visited dashboard yet
                ...
                const existingPost = (old?.posts || []).find((p: any) => p.id === newPost.id)
                const isNew = !existingPost
                const prevStatus = existingPost?.status
                const nextStatus = newPost.status
                ...
                return {
                    ...
                    stats: {
                        ...baseStats,
                        postsQueued: baseStats.postsQueued + 
                            (nextStatus === "SCHEDULED" ? 1 : 0) - 
                            (prevStatus === "SCHEDULED" ? 1 : 0),

                        totalPostsPublished: baseStats.totalPostsPublished + 
                            (nextStatus === "PUBLISHED" ? 1 : 0) - 
                            (prevStatus === "PUBLISHED" ? 1 : 0),

                        totalCount: baseStats.totalCount + (isNew ? 1 : 0),
                    },
                }
            })
            ...
            queryClient.setQueryData(["posts"], (old: any) => {
                ...
                const filtered = old.filter((p: any) => p.id !== newPost.id)
