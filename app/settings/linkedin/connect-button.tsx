...
export function ConnectLinkedInButton({ isConnected }: { isConnected: boolean }) {
  ...
  const handleDisconnect = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/auth/disconnect/linkedin", {
        method: "POST",
      });
  ...
  if (isConnected) {
    return (
      <div className="space-y-3">
        <Button
          variant="outline"
          className="w-full bg-slate-50 dark:bg-slate-800/50 border-green-200 dark:border-green-900/50 text-green-700 dark:text-green-400"
          disabled
        >
